...
from models.mlp import MLP, MLPTransformerEngine
...
class WindowMultiHeadAttentionTransformerEngine(nn.Module):
    ...
    def __init__(
        ...
        assert num_heads % tp_size == 0, "heads are not evenly split across TP model ranks"
...
class WindowMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.biasq = BiasLayer(dim_local, comm_metadata={
                "sharded": ["tp"],
                "shared": ["sp1-sp2"],
                "reduce": ["sp1-sp2"],
            },
        )
        self.biask = BiasLayer(dim_local, comm_metadata={
            "sharded": ["tp"],
            "shared": ["sp1-sp2"],
            "reduce": ["sp1-sp2"],
        })
        self.biasv = BiasLayer(dim_local, comm_metadata={
            "sharded": ["tp"],
            "shared": ["sp1-sp2"],
            "reduce": ["sp1-sp2"],
        })
...
class Block(nn.Module):
    def __init__(
        ...
        if use_transformer_engine:
            ...
            mlp_layer = MLPTransformerEngine
        else:
    ...
    def _make_attention_mask(self, T: int, causal: bool = False) -> None:
        ...
        temporal = temporal.expand(mask_shape + (self.window_area, self.window_area))  # mask_shape + (window_area, window_area)
        ...
        if any(self.shift_size):
            ...
            combined = temporal.expand(num_windows, *mask_shape, self.window_area, self.window_area)
